...
//测试通过
bool create_folder(char *user_name) 
{
  u8 res ;
  res = f_mkdir(user_name); 
  if (res == FR_OK)
  {
    return SUCCESS;
  }
...
//向指定用户创立文件 前提得找到当前用户的名字(根据日期来创建,只有有这个文件就返回成功)
//参数：file_name 用户文件名  user_name用户名
//返回：SUCCESS，成功 FAIL失败
//测试通过
bool create_file(char *user_name,char *file_name)
{
  u8 res ;
  char SD_juan[80] = "0:";  //SD卷号
  strcat(SD_juan,user_name);strcat(SD_juan,"/");
  strcat(SD_juan,file_name);
  res = f_open(&fnew,SD_juan, FA_OPEN_ALWAYS | FA_READ |FA_WRITE ); //创建文件
  if (res == FR_OK)
  {
    f_close(&fnew);  
    return SUCCESS;
...
//删除文件夹以及文件夹下面的文件
//参数：user_name 删除文件夹的名字
//返回：SUCCESS，成功 FAIL失败
//测试通过
bool del_user_info(char *user_name)
{
  u8 res;
  res = f_deldir(user_name);
  if(res == FR_OK)
  {
    return SUCCESS;
  }
...
//删除指定用户下面的文件
//参数：user_name 删除用户文件夹的名字，data  删除文件名
//返回：SUCCESS，成功 FAIL失败
//测试通过
bool  del_data_info(char *user_name,char *data)
{
    u8 rs=0;
    char road_line[50] ="0:";
    strcat(road_line,user_name);strcat(road_line,"/");
    strcat(road_line,data);
    rs = f_unlink(road_line);
    if(rs == FR_OK)
    {
        return SUCCESS;
    }
...
//向指定用户写文件,数据
//参数：user_name 用户名 file_name 用户文件名 write_data 写入的数据 Data_len 数据长度 (pointer为文件指针大小)
//返回：SUCCESS，成功 FAIL失败
//测试通过
bool write_file(char *user_name,char *file_name,char *write_data,uint8_t Data_len)
{
  u8 res ;
  char road_line[]="0:"; //存放路径
  strcat(road_line,user_name);strcat(road_line,"/");
  strcat(road_line,file_name);
  res = f_opendir(&Folder,user_name);
  if(res == FR_OK)//路径打开成功
  {
    res = f_open(&fnew,road_line, FA_OPEN_ALWAYS | FA_READ |FA_WRITE); //打开文件TXT
    if(res == FR_OK)//文件打开成功
    {
      //res = f_stat(road_line,&infotion); //读取文件信息（这个步凑可以通过f_open实现，后面可以不要）
      //if(res == FR_OK)
      //{
        res = f_lseek(&fnew,fnew.fsize); //移动指针到文件末尾
        if(res == FR_OK)//文件指针移动到最后成功
        {
          res = f_write(&fnew, write_data, Data_len, &bw); //写文件
          if(res == FR_OK)//文件写成功
          {
           // pointer+=Data_len;
            f_close(&fnew);  
